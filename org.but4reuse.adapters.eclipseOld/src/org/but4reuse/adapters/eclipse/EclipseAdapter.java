						...
						PluginElement fileElement = (PluginElement) element;
							 ...
							 // if (dir.getParent() != null) {
								...
								//	File file = FileUtils.getFile(fileElement.getAbsolutePath());
									...
									try {
										//URI uriPlug = url.toURI();
										 //File source = new File(uriPlug);
											System.out.println("URI : "+fileElement.getUri());
											
											File source = FileUtils.getFile(fileElement.getUri());
											if(source ==null)	System.out.println("fichier source ne se crée pas");
									//copyFile(source, f);
...
//			File file = FileUtils.getFile(fileElement.getAbsolutePath());
		...
		FileUtils.downloadFileFromURL(new URL("file:///"+pluginAddr), new File("C:/UPMC/M2/GPSTL/fevrier/runtime-EclipseApplication/test/plugins/"+pluginName));
